...
http {
    ...
    server {
        ...
        # T18：公开元数据（sitemap + llms + feed）— SEO 友好，无 auth
        # 用 alias 相对路径（nginx -p 在 deploy 时指向 release 根目录，www/ 在其下）
        location = /sitemap.xml       { auth_basic off; access_log off; alias www/sitemap.xml; }
        location = /sitemap.xml.gz   { auth_basic off; access_log off; alias www/sitemap.xml.gz; default_type application/gzip; }
        location = /llms.txt         { auth_basic off; access_log off; alias www/llms.txt; }
        location = /llms.txt.gz      { auth_basic off; access_log off; alias www/llms.txt.gz; default_type application/gzip; }
        location = /llms-full.txt    { auth_basic off; access_log off; alias www/llms-full.txt; }
        location = /llms-full.txt.gz { auth_basic off; access_log off; alias www/llms-full.txt.gz; default_type application/gzip; }
        location = /feed.xml         { auth_basic off; access_log off; alias www/feed.xml; }
        location = /feed.xml.gz      { auth_basic off; access_log off; alias www/feed.xml.gz; default_type application/gzip; }

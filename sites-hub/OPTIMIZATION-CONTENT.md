...
- **工作量**：4 小时
  ...
  - marker 保护：`<!-- auto-enrich:do-not-edit -->` 幂等
  - 干跑预览 → 写 → 重跑 audit 验证 score 提升
- **预期收益**：低完整度页数 -80（score ≤ 1 的"完全空白页"约 80 个）
- **依赖**：§8.65 脚本
...
- **背景**：audit baseline 已稳定，但缺历史趋势曲线，无法评估优化效果
- **目标**：每周一自动跑 audit，记录"低完整度 / dups / imgs / 跨站引用"4 指标变化曲线
- **工作量**：1 天
  - build-audit-dashboard.py：解析历史报告 → 输出 HTML dashboard
  - GitHub Action 周一 cron 自动跑
  - 数据存 `sites-hub/reports/history/audit-YYYYMMDD.json`
- **预期收益**：可视化优化进度，给 §8.62/§8.10 提供数据支撑
- **依赖**：audit-content.py 输出 JSON 格式（当前只输出 md）

#### §8.76 · 跨站引用密度补强

- **背景**：xsite 399 / 总 1.24M 字 = 0.32 链接/千字。低密度站（cloud 0.12 / design-pattern 0.16 / filesystem 0.18）拖后腿
- **目标**：低密度站（< 0.20）密度提升到 ≥ 0.30
- **工作量**：2 天
  - 扩 §8.60 xlink-injector 关键词映射（针对低密度站定制术语表）
  - 给 index.md 之外的关键页（如 cheatsheet.md / overview.md）也注入
- **预期收益**：低密度站搜索引擎可见性 +50%，跨站流量 +20%
- **依赖**：§8.60 脚本

#### §8.77 · C1 子站结构持续 review

- **背景**：C1 §8.50 已标准化 nav / head / sidebar 模板，但新增站（android/iot/game）后部分细节漂移
- **目标**：5 个新站（android/iot/game/system-design/springcloud）结构对齐到 28 老站规范
- **工作量**：1 天
  - render-config.py 统一处理所有站（包括新站）
  - sidebar 标准化：所有站用相同 H1/H2 层级（部分新站用了 H3 跳过 H2）
  - nav 项顺序统一（门户 → 首页 → 图谱 → 导图 → 速查 → 路径 → 更多）
- **预期收益**：用户体验一致，新站加入流程从"手工"变"自动"
- **依赖**：render-config.py 模板（§8.69 已加 MERMAID_SITES）

### 🟢 P2（长期 / 可选）

#### §8.78 · frontmatter date 全量补

- **背景**：1554 文件 FM 缺 date，VitePress `lastUpdated: true` 兜底用 git commit 时间，但 SEO/审计期望有显式 date
- **目标**：1554 文件全量加 `date:` 字段（用文件首次 commit 时间）
- **工作量**：4 小时（脚本批量 + git filter-repo 注入）
- **预期收益**：FM 完整度 100% → 100%，审计 baseline 数字更准确
- **风险**：可能破坏 git blame（用 filter-repo 后）

#### §8.79 · LLM 自动扩写低完整度页

- **背景**：294 低完整度页手动补全工作量大（每页 ~30 分钟 = 147 小时）
- **目标**：用 LLM（gpt-4 / claude-3.5）对 score = 0-1 的页自动生成"## 实战示例 / ## 常见问题"段
- **工作量**：3-5 天（含 prompt 工程 + 质量验证 + 人工 review）
- **预期收益**：低完整度页数 -50%，人力成本降低 70%
- **风险**：LLM 生成内容可能不准确，需人工 review（成本不可忽视）
- **前置**：成本评估（API 调用费 vs 人工成本）

#### §8.80 · 内容审核 SOP + 编辑规范

- **背景**：当前 PR review 流程依赖维护者个人判断，无统一标准
- **目标**：编写 CONTENT-STYLE.md（标题规范 / 章节模板 / 内链密度 / 图片要求）+ PR review checklist
- **工作量**：1 天
- **预期收益**：新贡献者上手快，内容质量一致性提升
- **依赖**：C10 CONTRIBUTING.md（已就位）

## 8.71.4 推荐路线（按 ROI 排序）

| 优先级 | 任务 | 工作量 | ROI |
| --- | --- | ---: | ---: |
| 🥇 | §8.74 enrich --apply | 4h | ★★★★★ |
| 🥈 | §8.73 dups 低频合并 | 2-3d | ★★★★ |
| 🥉 | §8.72 C11 图片优化 | 3-5d | ★★★★ |
| 4 | §8.76 跨站引用密度补强 | 2d | ★★★ |
| 5 | §8.75 C3 趋势 dashboard | 1d | ★★★ |
| 6 | §8.77 C1 子站结构 review | 1d | ★★ |
| 7 | §8.80 内容审核 SOP | 1d | ★★ |
| 8 | §8.78 frontmatter date | 4h | ★ |
| 9 | §8.79 LLM 自动扩写 | 3-5d | 待评估 |

## 8.71.5 与现有 P0-P2 计划的关系

```
P0（已完成大部分）：
  - §8.55 ✓ §8.60 ✓ §8.62 ✓ §8.63 ✓ §8.64 ✓ §8.65 ✓
  - §8.66 ✓ §8.67 ✓ §8.68 ✓ §8.69 ✓
P1（部分完成）：
  - C1 子站结构 → §8.77 继续
  - C2 跨站链接 → §8.76 继续
  - C3 趋势 → §8.75 启动
  - C4 Pagefind → 单独排期
  - C5 git log feed → ✓ §8.64
P2（待启动）：
  - C9 Plausible → 独立任务（隐私统计）
  - C10 PR review → §8.80 配套
  - C11 图片优化 → §8.72 P0
```

## 8.71.6 决策原则（沿用 §8.49 以来）

- **低投入高价值**：优先 4h-1d 任务，回避 ≥ 3d 重型投入
- **不动 CI**：除非必要，所有 CI 改动长期留工作区
- **只提交不推送**：commit 拆细，等用户明确 push
- **结构化输出**：故障按"根因-修复-预防"，文档按"背景-实施-复用"
- **低风险优先**：诊断 / 报告类先做，自动化注入类次之，LLM 类最后

## 8.71.7 下一步选择

- **选 1** → §8.74 enrich --apply（4h，最快见效）
- **选 2** → §8.73 dups 低频合并（2-3d，dups 数字大幅下降）
